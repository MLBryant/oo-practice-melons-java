...
public class Main {
	...
	public static void main(String[] args) {
		...
		Melon melon6 = new Melon(melonTypes[0], 7, 10, 3, "Michael");
	...
	public static void getSellabilityReport(Melon[] melons) {
		for ( Melon melon: melons) {
			if (melon.isSellable()) {
				System.out.printf("Harvested by %s from Field %s (CAN BE SOLD)%n", melon.harvester, melon.field);
			} else {
				System.out.printf("Harvested by %s from Field %s (NOT SELLABLE)%n", melon.harvester, melon.field);
			}
		}
	}
